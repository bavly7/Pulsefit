"""
food_logger.py — Egyptian Food Logger for AI Gym Trainer
All foods per 100g base. Integrates with user_profile.py for calorie targets.
Fully offline. Supports Arabic, Franco-Arabic, and English search.
 
COOKING METHOD SYSTEM:
- Foods with "ask_cooking": True → user picks how it was cooked (grilled/fried/boiled/roasted)
  and the system applies the correct calorie/fat multiplier.
- Foods that are already a complete dish (koshary, macarona bechamel, tarb…) → no cooking question.
"""
...
# ─────────────────────────────────────────────
#  FRANCO-ARABIC -> ENGLISH LOCAL MAP
# ─────────────────────────────────────────────
FRANCO_MAP = {
    # proteins
...
#
#  ask_cooking: True  -> raw ingredient, ask how it was cooked
#  ask_cooking: False -> complete dish, log as-is
...
FOOD_DB = [
    ...
    # Grilled/prepared meat dishes (already cooked)
    ...
    # Fish — complete (already grilled or fried as named)
...
# ─────────────────────────────────────────────
#  FRANCO / ARABIC -> ENGLISH NORMALIZER
# ─────────────────────────────────────────────
...
def _fuzzy(query: str) -> list:
    ...
    close = set(get_close_matches(query.lower().strip(),
                                  [c[0] for c in candidates], n=5, cutoff=0.75))
    seen, results = set(), []
...
def find_food(query: str):
    ...
    r = _fuzzy(query)
    if r:
        print("  [قريب منه] هل تقصد واحد من دول؟")
        return r, "fuzzy"
    return [], "none"
 
 
# ─────────────────────────────────────────────
#  COOKING METHOD PROMPT
# ─────────────────────────────────────────────
 
def ask_cooking_method() -> dict:
    print("\n  اتعمل إزاي؟")
...
def log_food_item(log: dict):
    print("\nSearch for food (or Enter to see all): ", end="")
    query = input().strip()
 
    if query == "":
        results = [f for f in FOOD_DB if not f.get("is_oil", False)]
    else:
        results, _ = find_food(query)
 
    if not results:
        print(f"  '{query}' مش موجود في قاعدة البيانات.")
        return
 
    # Pick food
    if len(results) == 1:
        food = results[0]
        print(f"  هل تقصد: {food['name']} ({food['calories']} kcal/100g)? (y/n): ", end="")
        if input().strip().lower() != "y":
            print("  Item not logged.")
            return
    else:
        shown = results[:20]
        for i, f in enumerate(shown, 1):
            tag = "  *" if f.get("ask_cooking") else ""
            print(f"    {i}. {f['name']} — {f['calories']} kcal/100g{tag}")
        if any(f.get("ask_cooking") for f in shown):
            print("  (* سيتم السؤال عن طريقة الطهي)")
        while True:
            try:
                choice = int(input("  Pick number: ").strip())
    ...
    # Cooking method (only for raw ingredients)
    extra_fat = extra_cal = extra_carb = 0
    method_label = ""
    if food.get("ask_cooking"):
        method = ask_cooking_method()
    ...
    # Grams
    print(f"  How many grams of {food['name']}{method_label}? ", end="")
    while True:
        try:
            grams = float(input().strip())
            if grams > 0:
                break
        except ValueError:
            pass
    ...
    print(f"\n  checkmark {grams}g {logged_name} -> {nutrients['calories']} kcal | "
...
def display_summary(log: dict, calorie_target: int):
    ...
    if log["entries"]:
        for i, e in enumerate(log["entries"], 1):
            print(f"    {i}. {e['food']} {e['grams']}g -> {e['calories']} kcal | "
