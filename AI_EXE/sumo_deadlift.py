...
class SumoDeadliftTrainerAI:
    def __init__(self, language="ar"):
        self.counter  = 0
        self.stage    = "down"        # "down" = hinge, "up" = lockout
        self.feedback = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        if confs[5] > 0.4 and confs[11] > 0.4 and confs[13] > 0.4 and confs[15] > 0.4:
            ...
            self.coach.on_error(SUMO_DEADLIFT_SETUP, urgent=True)
            return frame

        self.coach.reset_error(SUMO_DEADLIFT_SETUP)
        ...
        # Form: knees too bent
        if knee_angle < 120:
            self.feedback = "PUSH KNEES OUT!"
            current_color = (0, 0, 255)
            self.coach.on_error(SUMO_DEADLIFT_BACK)
        else:
            self.coach.reset_error(SUMO_DEADLIFT_BACK)
        ...
        if hip_angle > 165:
            if self.stage == "down":
                ...
                self.coach.reset_error(SUMO_DEADLIFT_LOCK)
                self._reset_rep_tracking()
                self.max_angle_this_rep = hip_angle
            else:
                if knee_angle >= 120:
                    self.feedback = "LOCKOUT!"
                    current_color = (0, 255, 0)
        ...
        # 2) Bottom (full hinge)
            ...
            if self.stage != "down":
                ...
                self.coach.reset_error(SUMO_DEADLIFT_PULL)
                self._reset_rep_tracking()
        ...
        # 3) Hysteresis: descending (stage=="up")
            ...
            if hip_angle - self.min_angle_this_rep > 15:
                self.feedback = "HINGE LOWER!"
                current_color = (0, 165, 255)
                self.coach.on_error(SUMO_DEADLIFT_PULL)
            else:
                if knee_angle >= 120:
                    self.feedback = "LOADING..."
                    current_color = (255, 255, 255)
                self.coach.reset_error(SUMO_DEADLIFT_PULL)
        ...
        # 4) Hysteresis: ascending (stage=="down")
            ...
            if self.max_angle_this_rep - hip_angle > 15:
                ...
                current_color = (0, 165, 255)
                self.coach.on_error(SUMO_DEADLIFT_LOCK)
            else:
                if knee_angle >= 120:
                    self.feedback = "PULLING UP..."
                    current_color = (255, 255, 255)
                self.coach.reset_error(SUMO_DEADLIFT_LOCK)
