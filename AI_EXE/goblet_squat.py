...
from AI_EXE.messages import (
    ...
    GOBLET_SQUAT_UP,
    GOBLET_SQUAT_DEPTH,
    GOBLET_SQUAT_CHEST, # 🟢 الرسالة الجديدة بتاعة الضهر
    GOOD_REP
)

class GobletSquatTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[11] > 0.4 and confs[13] > 0.4 and confs[15] > 0.4 and confs[5] > 0.4:
            ...
            self.coach.on_error(GOBLET_SQUAT_SETUP, urgent=True)
            return frame

        self.coach.reset_error(GOBLET_SQUAT_SETUP)
        ...
        # ── FORM CHECK: BACK POSTURE (استقامة الضهر) ──
        if hip_angle < 45:
            self.feedback = "KEEP CHEST UP!"
            current_color = (0, 0, 255)
            self.coach.on_error(GOBLET_SQUAT_CHEST) # 👈 هيزعقله ويقوله ارفع صدرك
        else:
            self.coach.reset_error(GOBLET_SQUAT_CHEST)
            ...
            if knee_angle < 95:
                if self.stage != "down":
                    ...
                    self.coach.reset_error(GOBLET_SQUAT_DEPTH)
                    self._reset_rep_tracking()
                    self.min_angle_this_rep = knee_angle  
            ...
            # ── 2) Top (Stand up) ──
                ...
                if self.stage == "down":
                    ...
                    self.coach.reset_error(GOBLET_SQUAT_UP)
                    self.coach.reset_error(GOBLET_SQUAT_DEPTH)

                    self._reset_rep_tracking()
                    self.max_angle_this_rep = knee_angle  

            # ── 3) Hysteresis: Short Rep while descending (نص نازلة) ──
        ...
        cv2.line(frame, tuple(shoulder.astype(int)), tuple(hip.astype(int)), (255, 0, 0), 2) # خط الضهر باللون الأزرق عشان يبان
