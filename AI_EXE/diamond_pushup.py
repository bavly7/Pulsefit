...
from AI_EXE.messages import (
    DIAMOND_PUSHUP_SETUP,
    DIAMOND_PUSHUP_LOWER,
    DIAMOND_PUSHUP_PUSH,
    DIAMOND_PUSHUP_WIDE,
    GOOD_REP # 🟢 عشان التشجيع
)

class DiamondPushupTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "up"  # "up" = دراع مفرود, "down" = دراع متني والصدر قريب من الإيد
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(DIAMOND_PUSHUP_SETUP, urgent=True)
            return frame

        self.coach.reset_error(DIAMOND_PUSHUP_SETUP)
        ...
        # ── ELBOW FLARE CHECK ────────────────────────────────────────
        # الدايموند بوش أب لازم الكوع يكون مضموم على الجسم، لو فتح لبرة يبقى غلط
        upper_arm_angle = calculate_angle(hip, shoulder, elbow)
        if upper_arm_angle > 45:
            self.feedback = "TUCK ELBOWS IN!"
            current_color = (0, 0, 255)
            self.coach.on_error(DIAMOND_PUSHUP_WIDE)
        else:
            self.coach.reset_error(DIAMOND_PUSHUP_WIDE)
            ...
            if angle > 160:
                if self.stage != "up":
                    ...
                    self.coach.reset_error(DIAMOND_PUSHUP_LOWER)
                    self.coach.reset_error(DIAMOND_PUSHUP_PUSH)
                    self.coach.reset_error(DIAMOND_PUSHUP_WIDE)
                    
                    self._reset_rep_tracking()

            # 2) Bottom (chest to hands)
            elif angle < 70:
                if self.stage != "down":
                    self.stage = "down"
            ...
            elif self.stage == "up" and 70 <= angle <= 160:
                # لو عكس اتجاهه لفوق 15 درجة قبل ما يوصل للأرض
                if angle - self.min_angle_this_rep > 15:
                    self.feedback = "LOWER TO HANDS!"
                    current_color = (0, 165, 255)
                    voice_message = DIAMOND_PUSHUP_LOWER
                else:
                    self.feedback = "LOWERING..."
                    current_color = (255, 255, 255)

            # 4) Warning: Partial lock-out while ascending
            elif self.stage == "down" and 70 <= angle <= 160:
                # لو عكس اتجاهه لتحت 15 درجة قبل ما يفرد دراعه للآخر
                if self.max_angle_this_rep - angle > 15:
                    self.feedback = "PUSH UP FULLY!"
                    current_color = (0, 165, 255)
                    voice_message = DIAMOND_PUSHUP_PUSH
                else:
