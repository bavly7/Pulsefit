...
from AI_EXE.messages import (
    STANDING_CALF_RAISE_SETUP,
    STANDING_CALF_RAISE_RAISE,
    STANDING_CALF_RAISE_SQUEEZE,
    STANDING_CALF_RAISE_LOWER,
    GOOD_REP,
)


class StandingCalfRaiseTrainerAI:
    """
    ...
    def __init__(self, language="ar"):
        self.counter  = 0
        self.stage    = "down"        # "down" = heel flat/dropped, "up" = heel raised
        self.feedback = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        # Right side: knee=14, ankle=16, foot_index=32
            ...
            self.coach.on_error(STANDING_CALF_RAISE_SETUP, urgent=True)
            return frame

        self.coach.reset_error(STANDING_CALF_RAISE_SETUP)
        ...
        # 1) Top (heel fully raised — plantar flexion peak)
        if ankle_angle > 130:
            if self.stage == "down":
                self.stage    = "up"
                self.counter += 1
                ...
                self.coach.reset_error(STANDING_CALF_RAISE_RAISE)
        ...
        # 2) Bottom (heel fully lowered — full dorsiflexion stretch)
        elif ankle_angle < 90:
            if self.stage != "down":
                self.stage    = "down"
                self.feedback = "RAISE YOUR HEELS!"
                current_color = (255, 255, 255)
                self.coach.reset_error(STANDING_CALF_RAISE_LOWER)
                self._reset_rep_tracking()
                self.min_angle_this_rep = ankle_angle

        # 3) Hysteresis: descending (stage=="up", mid-range)
        elif self.stage == "up" and 90 <= ankle_angle <= 130:
            # Reversal: was decreasing but went UP > 15° without reaching full stretch
            if ankle_angle - self.min_angle_this_rep > 15:
                self.feedback = "LOWER FULLY FOR STRETCH!"
                current_color = (0, 165, 255)
                self.coach.on_error(STANDING_CALF_RAISE_LOWER)
            else:
                self.feedback = "LOWERING..."
                current_color = (255, 255, 255)
                self.coach.reset_error(STANDING_CALF_RAISE_LOWER)

        # 4) Hysteresis: ascending (stage=="down", mid-range)
        elif self.stage == "down" and 90 <= ankle_angle <= 130:
            # Reversal: was increasing but DROPPED > 15° without reaching peak
            if self.max_angle_this_rep - ankle_angle > 15:
                self.feedback = "RAISE HEELS HIGHER!"
                current_color = (0, 165, 255)
                self.coach.on_error(STANDING_CALF_RAISE_RAISE)
            else:
