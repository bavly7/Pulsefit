...
from AI_EXE.messages import (
    LATERAL_LANDMINE_SETUP,
    LATERAL_LANDMINE_RAISE,
    LATERAL_LANDMINE_LOWER,
    LATERAL_LANDMINE_TOO_HIGH,
    GOOD_REP,
)

class LandmineLateralRaiseTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(LATERAL_LANDMINE_SETUP, urgent=True)
            return frame

        self.coach.reset_error(LATERAL_LANDMINE_SETUP)
        ...
        if current_angle > 110:
            ...
            self.coach.on_error(LATERAL_LANDMINE_TOO_HIGH)
        else:
            self.coach.reset_error(LATERAL_LANDMINE_TOO_HIGH)
        ...
        if current_angle > 85:
            if self.stage == "down":
                ...
                self.coach.reset_error(LATERAL_LANDMINE_RAISE)
                self.coach.reset_error(LATERAL_LANDMINE_LOWER)
        ...
        # ── 2) Bottom (Arms down) ──
            ...
            if self.stage != "down":
                self.stage = "down"
                self.feedback = "PUSH BAR UP AND OUT!"
        ...
        # ── 3) Hysteresis: Ascending (stage=="down") ──
            ...
            if self.max_angle_this_rep - current_angle > 15:
                ...
                voice_message = LATERAL_LANDMINE_RAISE
            else:
        ...
        # ── 4) Hysteresis: Descending (stage=="up") ──
            ...
            if current_angle - self.min_angle_this_rep > 15:
                ...
                current_color = (0, 165, 255)
                voice_message = LATERAL_LANDMINE_LOWER
            else:
