...
class LatPulldownUnderhandTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "up"  # "up" = full stretch, "down" = pulled down
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(LAT_UNDER_SETUP, urgent=True)
            return frame

        self.coach.reset_error(LAT_UNDER_SETUP)
        ...
        if angle < 70:
            if self.stage != "down":
                ...
                self.coach.reset_error(LAT_UNDER_PULL)
                self.coach.reset_error(LAT_UNDER_STRETCH)
                self.coach.reset_error(LAT_UNDER_SQUEEZE)
                
                self._reset_rep_tracking()

        # 2) Full stretch (up) - Elbow straight
        elif angle > 155:
            if self.stage != "up":
                self.stage = "up"
