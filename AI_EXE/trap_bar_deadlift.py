...
class TrapBarDeadliftTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[12] > 0.4 and confs[14] > 0.4 and confs[16] > 0.4:
            ...
            self.coach.on_error(TRAP_DEADLIFT_SETUP, urgent=True)
            return frame

        self.coach.reset_error(TRAP_DEADLIFT_SETUP)
        ...
        if angle > 165:
            if self.stage != "up":
                ...
                self.coach.reset_error(TRAP_DEADLIFT_PULL)
                self.coach.reset_error(TRAP_DEADLIFT_LOCK)
                self.coach.reset_error(TRAP_DEADLIFT_DROP)
        ...
        # 3) Warning: Not pulling all the way to lockout
            ...
            if self.max_angle_this_rep - angle > 15:
                ...
                voice_message = TRAP_DEADLIFT_LOCK
            else:
        ...
        # 4) Warning: Partial drop while returning down
            ...
            if angle - self.min_angle_this_rep > 15:
                ...
                current_color = (0, 165, 255)
                voice_message = TRAP_DEADLIFT_DROP
            else:
