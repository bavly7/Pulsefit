...
class MachineAssistedPullUpTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(AST_PULLUP_SETUP, urgent=True)
            return frame

        self.coach.reset_error(AST_PULLUP_SETUP)
        ...
        if angle < 70:
            if self.stage != "up":
                ...
                self.coach.reset_error(AST_PULLUP_PULL)
                self.coach.reset_error(AST_PULLUP_HANG)
                self.coach.reset_error(AST_PULLUP_CHIN)
                
                self._reset_rep_tracking()

        # 2) Full hang (down) - Elbow straight
        elif angle > 155:
            if self.stage != "down":
                self.stage = "down"
        ...
        # 3) Warning: Not pulling high enough
            ...
            if angle - self.min_angle_this_rep > 15:
                self.feedback = "PULL CHIN HIGHER!"
                current_color = (0, 165, 255)
                voice_message = AST_PULLUP_PULL
            else:
