...
class SeatedCableRowTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "forward"  # "forward" = full stretch, "back" = pulled in
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(CABLE_ROW_SETUP, urgent=True)
            return frame

        self.coach.reset_error(CABLE_ROW_SETUP)
        ...
        if angle < 70:
            if self.stage != "back":
                ...
                self.coach.reset_error(CABLE_ROW_PULL)
                self.coach.reset_error(CABLE_ROW_STRETCH)
                self.coach.reset_error(CABLE_ROW_SQUEEZE)
        ...
        # 4) Warning: Partial stretch while returning
            ...
            if self.max_angle_this_rep - angle > 15:
                ...
                current_color = (0, 165, 255)
                voice_message = CABLE_ROW_STRETCH
            else:
