...
from AI_EXE.messages import (
    BARBELL_BACK_SQUAT_SETUP,
    BARBELL_BACK_SQUAT_LOWER,
    BARBELL_BACK_SQUAT_UP,
    BARBELL_BACK_SQUAT_DEPTH,
    BARBELL_BACK_SQUAT_KNEE,
    GOOD_REP
)

class BarbellBackSquatTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        if confs[11] > 0.4 and confs[13] > 0.4 and confs[15] > 0.4:
            s_idx, h_idx, k_idx, a_idx = 5, 11, 13, 15
        elif confs[10] > 0.4 and confs[12] > 0.4 and confs[14] > 0.4:
            s_idx, h_idx, k_idx, a_idx = 6, 10, 12, 14
        else:
            self.feedback = "SHOW SIDE VIEW"
            self._draw_feedback(frame, (0, 0, 255))
            self.coach.on_error(BARBELL_BACK_SQUAT_SETUP, urgent=True)
            return frame

        self.coach.reset_error(BARBELL_BACK_SQUAT_SETUP)
        ...
        hip_angle = calculate_angle(shoulder, hip, knee) # Form tracker
        ...
        # لو ميل لقدام أوي (زاوية الوسط قفلت)
        if hip_angle < 45:
            self.feedback = "KEEP CHEST UP!"
            current_color = (0, 0, 255)
            # ممكن تضيف إيرور مخصوص للضهر هنا لو عندك في الـ messages
        ...
        if knee_angle < 95:
            if self.stage != "down":
                ...
                self.coach.reset_error(BARBELL_BACK_SQUAT_DEPTH)
        ...
        # ── 2) Top (Stand up) ──
            ...
            if self.stage == "down":
                ...
                self.coach.reset_error(BARBELL_BACK_SQUAT_UP)
                self.coach.reset_error(BARBELL_BACK_SQUAT_DEPTH)
        ...
        # ── 3) Hysteresis: Short Rep while descending ──
            ...
            if knee_angle - self.min_angle_this_rep > 15:
                ...
                voice_message = BARBELL_BACK_SQUAT_DEPTH
            else:
        ...
        # ── 4) Hysteresis: Short Rep while ascending ──
            ...
            if self.max_angle_this_rep - knee_angle > 15:
                ...
                current_color = (0, 165, 255)
                voice_message = BARBELL_BACK_SQUAT_UP
            else:
