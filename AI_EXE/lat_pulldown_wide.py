...
class LatPulldownWideTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(LAT_WIDE_SETUP, urgent=True)
            return frame

        self.coach.reset_error(LAT_WIDE_SETUP)
        ...
        if angle < 70:
            if self.stage != "down":
                ...
                self.coach.reset_error(LAT_WIDE_PULL)
                self.coach.reset_error(LAT_WIDE_STRETCH)
                self.coach.reset_error(LAT_WIDE_SQUEEZE)
        ...
        # 4) Warning: Partial stretch while returning up
            ...
            if self.max_angle_this_rep - angle > 15:
                ...
                current_color = (0, 165, 255)
                voice_message = LAT_WIDE_STRETCH
            else:
