...
from AI_EXE.messages import (
    ...
    LANDMINE_PRESS_LOWER,
    LANDMINE_PRESS_LOCK,
    LANDMINE_PRESS_DROP,
    GOOD_REP # 🟢 للتشجيع
)

class LandminePressTrainerAI:
    def __init__(self, language="ar"):
        self.counter        = 0
        self.stage          = "up"  # "up" = البار مرفوع, "down" = البار قريب من الصدر
        self.feedback       = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            self.coach.on_error(LANDMINE_PRESS_SETUP, urgent=True)
            return frame
        
        self.coach.reset_error(LANDMINE_PRESS_SETUP)
        ...
        if self.stage == "up":
            # وهو بينزل بالبار (الزاوية بتقل)
            self.min_angle_this_rep = min(self.min_angle_this_rep, angle)
        elif self.stage == "down":
            # وهو بيزق لفوق (الزاوية بتزيد)
            self.max_angle_this_rep = max(self.max_angle_this_rep, angle)

        current_color = (0, 255, 0)
        voice_message = None

        # 1) Peak / lock-out (arms extended, but not fully to protect shoulder in landmine)
        if angle > 150:
            if self.stage != "up":
                self.stage    = "up"
                self.counter += 1
                ...
                self.coach.reset_error(LANDMINE_PRESS_LOCK)
                self.coach.reset_error(LANDMINE_PRESS_LOWER)
                self.coach.reset_error(LANDMINE_PRESS_DROP)
                
                self._reset_rep_tracking()

        # 2) Bottom (Bar close to chest)
        elif angle < 90:
            # 🟢 حماية الكتف من النزول المبالغ فيه
            if angle < 65:
                self.feedback = "DON'T DROP TOO LOW!"
                current_color = (0, 165, 255)
                voice_message = LANDMINE_PRESS_DROP
            else:
                self.coach.reset_error(LANDMINE_PRESS_DROP)
        ...
        # 3) Warning: Not lowering enough (Short rep while descending)
            ...
            # لو عكس اتجاهه لفوق بـ 15 درجة قبل ما يوصل للمستوى الصح
            if angle - self.min_angle_this_rep > 15:
                self.feedback = "LOWER TO CHEST!"
                current_color = (0, 165, 255)
                voice_message = LANDMINE_PRESS_LOWER
            else:
        ...
        # 4) Warning: Partial lock-out while ascending
            ...
            # لو عكس اتجاهه لتحت بـ 15 درجة قبل ما يزق البار لفوق
            if self.max_angle_this_rep - angle > 15:
                self.feedback = "LOCK IT OUT!"
                current_color = (0, 165, 255)
                voice_message = LANDMINE_PRESS_LOCK
            else:
