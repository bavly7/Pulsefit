...
class FlatDumbbellPressTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            self.coach.on_error(FLAT_PRESS_SETUP, urgent=True)
            return frame
        
        self.coach.reset_error(FLAT_PRESS_SETUP)
        ...
        if angle > 155:
            if self.stage != "up":
                ...
                self.coach.reset_error(FLAT_PRESS_LOCK)
                self.coach.reset_error(FLAT_PRESS_LOWER)
                self.coach.reset_error(FLAT_PRESS_DROP)
        ...
        # 3) Warning: Not lowering enough (Short rep while descending)
            ...
            if angle - self.min_angle_this_rep > 15:
                ...
                current_color = (0, 165, 255)
                voice_message = FLAT_PRESS_LOWER
            else:
