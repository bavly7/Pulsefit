...
from AI_EXE.messages import (
    INCLINE_DB_CURL_SETUP,
    INCLINE_DB_CURL_CURL,
    INCLINE_DB_CURL_STRETCH,
    INCLINE_DB_CURL_DROP,
    GOOD_REP # 🟢 عشان التشجيع
)

class InclineDumbbellCurlTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "down"  # "down" = دراع مفرود لتحت, "up" = متني لفوق
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(INCLINE_DB_CURL_SETUP, urgent=True)
            return frame

        self.coach.reset_error(INCLINE_DB_CURL_SETUP)
        ...
        if self.stage == "down":
            # طالع لفوق (الزاوية بتقل) -> نسجل أقل زاوية وصلها
            self.min_angle_this_rep = min(self.min_angle_this_rep, angle)
        elif self.stage == "up":
            # نازل لتحت (الزاوية بتزيد) -> نسجل أكبر زاوية وصلها
            self.max_angle_this_rep = max(self.max_angle_this_rep, angle)

        current_color = (0, 255, 0)
        voice_message = None

        # ── UPPER ARM FORM CHECK (Incline Bench Logic) ───────────────
        upper_arm_angle = calculate_angle(hip, shoulder, elbow)
        # في الانكلاين طبيعي الزاوية تكون كبيرة، الغلط إنها تعدى 75 (معناه بيرفع كوعه لقدام)
        if upper_arm_angle > 75:
            self.feedback = "KEEP ELBOWS POINTED DOWN!"
            current_color = (0, 0, 255)
            self.coach.on_error(INCLINE_DB_CURL_SETUP)
        else:
            self.coach.reset_error(INCLINE_DB_CURL_SETUP)
            ...
            if angle < 50:
                if self.stage != "up":
                    ...
                    self.coach.reset_error(INCLINE_DB_CURL_CURL)
                    self.coach.reset_error(INCLINE_DB_CURL_STRETCH)
                    self.coach.reset_error(INCLINE_DB_CURL_DROP)
            ...
            # 3) Warning: Not curling high enough (short rep while ascending)
                ...
                # لو الزاوية رجعت كبرت 15 درجة (يعني نزل الدمبل قبل ما يكمل رفعة)
                if angle - self.min_angle_this_rep > 15:
                    self.feedback = "CURL UP HIGH!"
                    current_color = (0, 165, 255)
                    voice_message = INCLINE_DB_CURL_CURL
                else:
                    self.feedback = "CURLING..."
                    current_color = (255, 255, 255)

            # 4) Warning: Partial extension while descending
            elif self.stage == "up" and 90 < angle < 155:
                # لو الزاوية صغرت 15 درجة (يعني بدأ يرفع تاني قبل ما يفرد للاخر)
                if self.max_angle_this_rep - angle > 15:
                    self.feedback = "LOWER FOR DEEP STRETCH!"
                    current_color = (0, 165, 255)
                    voice_message = INCLINE_DB_CURL_STRETCH
                else:
