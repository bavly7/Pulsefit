...
class ChestSupportedRowTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "forward"  # "forward" = full stretch, "back" = pulled up
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(CHEST_ROW_SETUP, urgent=True)
            return frame

        self.coach.reset_error(CHEST_ROW_SETUP)
        ...
        if angle < 70:
            if self.stage != "back":
                ...
                self.coach.reset_error(CHEST_ROW_PULL)
                self.coach.reset_error(CHEST_ROW_STRETCH)
                self.coach.reset_error(CHEST_ROW_SQUEEZE)
                
                self._reset_rep_tracking()

        # 2) Full stretch (forward) - Elbow straight
        elif angle > 155:
            if self.stage != "forward":
                self.stage = "forward"
        ...
        # 3) Warning: Not pulling all the way back
        elif self.stage == "forward" and 70 <= angle <= 155:
            if angle - self.min_angle_this_rep > 15:
                self.feedback = "PULL ALL THE WAY BACK!"
                current_color = (0, 165, 255)
                voice_message = CHEST_ROW_PULL
            else:
                self.feedback = "PULLING..."
                current_color = (255, 255, 255)

        # 4) Warning: Partial stretch while returning
        elif self.stage == "back" and 70 <= angle <= 155:
            if self.max_angle_this_rep - angle > 15:
                self.feedback = "STRETCH FULLY FORWARD!"
                current_color = (0, 165, 255)
                voice_message = CHEST_ROW_STRETCH
            else:
