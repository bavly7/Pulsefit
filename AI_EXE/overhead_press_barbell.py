...
class OverheadPressBarbellTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            self.coach.on_error(OHP_BARBELL_SETUP, urgent=True)
            return frame

        self.coach.reset_error(OHP_BARBELL_SETUP)
        ...
        if current_angle > 160:
            if self.stage == "down":
                ...
                self.coach.reset_error(OHP_BARBELL_LOCK)
                self.coach.reset_error(OHP_BARBELL_LOWER)
                self.coach.reset_error(OHP_BARBELL_DROP)
        ...
        # ── 3) Hysteresis: Ascending (stage=="down") ──
            ...
            if self.max_angle_this_rep - current_angle > 15:
                ...
                voice_message = OHP_BARBELL_LOCK
            else:
        ...
        # ── 4) Hysteresis: Descending (stage=="up") ──
            ...
            if current_angle - self.min_angle_this_rep > 15:
                ...
                current_color = (0, 165, 255)
                voice_message = OHP_BARBELL_LOWER
            else:
