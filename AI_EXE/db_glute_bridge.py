...
from AI_EXE.messages import (
    DB_GLUTE_BRIDGE_SETUP,
    DB_GLUTE_BRIDGE_UP,
    DB_GLUTE_BRIDGE_TOP,
    DB_GLUTE_BRIDGE_LOWER,
    GOOD_REP,
)


class DbGluteBridgeTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        if confs[5] > 0.4 and confs[11] > 0.4 and confs[13] > 0.4 and confs[15] > 0.4:
            ...
            self.coach.on_error(DB_GLUTE_BRIDGE_SETUP, urgent=True)
            return frame

        self.coach.reset_error(DB_GLUTE_BRIDGE_SETUP)
        ...
        if hip_angle > 160:
            if self.stage == "down":
                ...
                self.coach.reset_error(DB_GLUTE_BRIDGE_UP)
        ...
        # 2) Bottom (hips fully lowered)
            ...
            if self.stage != "down":
                ...
                self.coach.reset_error(DB_GLUTE_BRIDGE_TOP)
                self._reset_rep_tracking()
        ...
        # 3) Hysteresis: descending (stage=="up")
            ...
            if hip_angle - self.min_angle_this_rep > 15:
                ...
                self.coach.on_error(DB_GLUTE_BRIDGE_LOWER)
            else:
                self.feedback = "LOWERING..."
                current_color = (255, 255, 255)
                self.coach.reset_error(DB_GLUTE_BRIDGE_LOWER)
        ...
        # 4) Hysteresis: ascending (stage=="down")
            ...
            if self.max_angle_this_rep - hip_angle > 15:
                ...
                current_color = (0, 165, 255)
                self.coach.on_error(DB_GLUTE_BRIDGE_UP)
            else:
