...
from AI_EXE.messages import (
    PREACHER_CURL_SETUP,
    PREACHER_CURL_CURL,
    PREACHER_CURL_STRETCH,
    PREACHER_CURL_PIN,
    GOOD_REP # 🟢 عشان التشجيع
)

class PreacherCurlTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "down"  # "down" = دراع مفرود على المسند, "up" = دراع متني للاخر
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(PREACHER_CURL_SETUP, urgent=True)
            return frame

        self.coach.reset_error(PREACHER_CURL_SETUP)
        ...
        # ── UPPER ARM FORM CHECK (Preacher Bench Logic) ──────────────
        upper_arm_angle = calculate_angle(hip, shoulder, elbow)
        # في البرايتشر طبيعي الزاوية تكون كبيرة (معدية الـ 60)، الغلط لو رفع كوعه عن المسند (الزاوية تزيد عن 90)
        if upper_arm_angle > 95:
            self.feedback = "KEEP ELBOWS ON PAD!"
            current_color = (0, 0, 255)
            self.coach.on_error(PREACHER_CURL_PIN) # بنستخدم Pin للتعبير عن تثبيت الكوع
        else:
            self.coach.reset_error(PREACHER_CURL_PIN)
            ...
            if angle < 50:
                if self.stage != "up":
                    ...
                    self.coach.reset_error(PREACHER_CURL_CURL)
                    self.coach.reset_error(PREACHER_CURL_STRETCH)
                    
                    self._reset_rep_tracking()

            # 2) Bottom (arm fully extended on pad)
            elif angle > 155:
                if self.stage != "down":
                    self.stage = "down"
            ...
            # 3) Warning: Not curling high enough (short rep while ascending)
                ...
                # لو نزل الدمبل تاني قبل ما يكمل الرفعة (الزاوية كبرت 15 درجة)
                if angle - self.min_angle_this_rep > 15:
                    self.feedback = "CURL UP HIGH!"
                    current_color = (0, 165, 255)
                    voice_message = PREACHER_CURL_CURL
                else:
            ...
            # 4) Warning: Partial extension while descending
                ...
                # لو سحب الدمبل تاني قبل ما يفرد دراعه للاخر (الزاوية صغرت 15 درجة)
                if self.max_angle_this_rep - angle > 15:
                    self.feedback = "LOWER FULLY!"
                    current_color = (0, 165, 255)
                    voice_message = PREACHER_CURL_STRETCH
                else:
