...
from AI_EXE.messages import (
    DONKEY_CALF_RAISE_SETUP,
    DONKEY_CALF_RAISE_RAISE,
    DONKEY_CALF_RAISE_SQUEEZE,
    DONKEY_CALF_RAISE_LOWER,
    GOOD_REP,
)


class DonkeyCalfRaiseTrainerAI:
    """
    ...
    def process(self, frame, keypoints, confs):
        ...
        # Right side: knee=14, ankle=16, foot_index=32
            ...
            self.coach.on_error(DONKEY_CALF_RAISE_SETUP, urgent=True)
            return frame

        self.coach.reset_error(DONKEY_CALF_RAISE_SETUP)
        ...
        if ankle_angle > 130:
            if self.stage == "down":
                ...
                self.coach.reset_error(DONKEY_CALF_RAISE_RAISE)
        ...
        # 2) Bottom (heel fully lowered / full stretch)
            ...
            if self.stage != "down":
                self.stage    = "down"
                self.feedback = "RAISE YOUR HEELS HIGH!"
                current_color = (255, 255, 255)
                self.coach.reset_error(DONKEY_CALF_RAISE_LOWER)
                self._reset_rep_tracking()
        ...
        # 3) Hysteresis: descending (stage=="up")
            ...
            if ankle_angle - self.min_angle_this_rep > 15:
                ...
                self.coach.on_error(DONKEY_CALF_RAISE_LOWER)
            else:
        ...
        # 4) Hysteresis: ascending (stage=="down")
            ...
            if self.max_angle_this_rep - ankle_angle > 15:
                ...
                current_color = (0, 165, 255)
                self.coach.on_error(DONKEY_CALF_RAISE_RAISE)
            else:
