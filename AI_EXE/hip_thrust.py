...
class HipThrustTrainerAI:
    def __init__(self, language="ar"):
        self.counter  = 0
        self.stage    = "down"        # "down" = hips low, "up" = full extension
        self.feedback = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        # Hip angle needs shoulder, hip, knee
        if confs[5] > 0.4 and confs[11] > 0.4 and confs[13] > 0.4 and confs[15] > 0.4:
            ...
            self.coach.on_error(HIP_THRUST_SETUP, urgent=True)
            return frame

        self.coach.reset_error(HIP_THRUST_SETUP)
        ...
        if hip_angle > 160:
            if self.stage == "down":
                ...
                self.coach.reset_error(HIP_THRUST_THRUST)
