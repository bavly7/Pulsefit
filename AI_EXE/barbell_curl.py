...
from AI_EXE.messages import (
    BARBELL_CURL_SETUP,
    BARBELL_CURL_CURL,
    BARBELL_CURL_STRETCH,
    BARBELL_CURL_SWING,
    GOOD_REP # 🟢 عشان التشجيع
)

class BarbellCurlTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "down"  # "down" = دراع مفرود, "up" = دراع متني
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(BARBELL_CURL_SETUP, urgent=True)
            return frame

        self.coach.reset_error(BARBELL_CURL_SETUP)
        ...
        if self.stage == "down":
            # بيرفع البار لفوق (الزاوية بتقل) -> نسجل أقل زاوية
            self.min_angle_this_rep = min(self.min_angle_this_rep, angle)
        elif self.stage == "up":
            # بينزل البار (الزاوية بتزيد) -> نسجل أكبر زاوية
            self.max_angle_this_rep = max(self.max_angle_this_rep, angle)

        current_color = (0, 255, 0)
        voice_message = None

        # ── UPPER ARM FORM CHECK (منع المرجحة) ────────────────────────
        upper_arm_angle = calculate_angle(hip, shoulder, elbow)
        if upper_arm_angle > 45:
            self.feedback = "DON'T SWING YOUR ELBOWS!"
            current_color = (0, 0, 255)
            self.coach.on_error(BARBELL_CURL_SWING)
        else:
            self.coach.reset_error(BARBELL_CURL_SWING)
            ...
            if angle < 50:
                if self.stage != "up":
                    ...
                    self.coach.reset_error(BARBELL_CURL_CURL)
                    self.coach.reset_error(BARBELL_CURL_STRETCH)
                    self.coach.reset_error(BARBELL_CURL_SWING)
            ...
            # 3) Warning: Not curling high enough (short rep while ascending)
                ...
                # لو نزل البار قبل ما يكمل رفعة (الزاوية كبرت 15 درجة)
                if angle - self.min_angle_this_rep > 15:
                    self.feedback = "CURL TO CHEST!"
                    current_color = (0, 165, 255)
                    voice_message = BARBELL_CURL_CURL
                else:
            ...
            # 4) Warning: Partial extension while descending
                ...
                # لو رفع البار قبل ما يفرد دراعه للاخر (الزاوية صغرت 15 درجة)
                if self.max_angle_this_rep - angle > 15:
                    self.feedback = "LOWER FULLY!"
                    current_color = (0, 165, 255)
                    voice_message = BARBELL_CURL_STRETCH
                else:
