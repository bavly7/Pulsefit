...
class InclineBarbellPressTrainerAI:
    def __init__(self, language="ar"):
        self.counter        = 0
        self.stage          = "up"  # "up" = البار فوق مفرود, "down" = البار لامس الصدر
        self.feedback       = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            self.coach.on_error(INCLINE_BB_SETUP, urgent=True)
            return frame
        
        self.coach.reset_error(INCLINE_BB_SETUP)
        ...
        if self.stage == "up":
            # وهو بينزل بالبار (الزاوية بتقل) نراقب المينيمم
            self.min_angle_this_rep = min(self.min_angle_this_rep, angle)
        elif self.stage == "down":
            # وهو بيزق البار لفوق (الزاوية بتزيد) نراقب الماكسيمم
        ...
        if angle > 155:
            if self.stage != "up":
                ...
                self.coach.reset_error(INCLINE_BB_LOCK)
                self.coach.reset_error(INCLINE_BB_LOWER)
                self.coach.reset_error(INCLINE_BB_TOUCH)
                
                self._reset_rep_tracking()

        # 2) Bottom (Bar touches chest)
        # 🟢 رفعنا الزاوية لـ 95 عشان العدة تتحسب أسهل مع مسكات البار الواسعة
        elif angle < 95:
            if self.stage != "down":
                self.stage = "down"
        ...
        # 3) Warning: Not lowering enough (Short rep while descending)
            ...
            # 🟢 لو هو نازل وقرر يعكس اتجاهه ويطلع بـ 15 درجة قبل ما يلمس صدره
            if angle - self.min_angle_this_rep > 15:
                self.feedback = "TOUCH THE CHEST!"
                current_color = (0, 165, 255)
                voice_message = INCLINE_BB_TOUCH
            else:
