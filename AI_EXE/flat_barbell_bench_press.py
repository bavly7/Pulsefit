...
class FlatBarbellBenchPressTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            self.coach.on_error(FLAT_BB_SETUP, urgent=True)
            return frame
        
        self.coach.reset_error(FLAT_BB_SETUP)
        ...
        if angle > 155:
            if self.stage != "up":
                ...
                self.coach.reset_error(FLAT_BB_LOCK)
                self.coach.reset_error(FLAT_BB_LOWER)
                self.coach.reset_error(FLAT_BB_TOUCH)
                
                self._reset_rep_tracking()

        # 2) Bottom (Bar touches chest)
        # 95 ممتازة لأن البار لما بيلمس الصدر الكوع بينزل تحت مستوى الجسم شوية
        elif angle < 95:
            if self.stage != "down":
                self.stage = "down"
