...
class ArnoldPressTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            s_idx, s_idx, w_idx = 5, 7, 9
        else:
            self.feedback = "SHOW SIDE VIEW"
            self._draw_feedback(frame, (0, 0, 255))
            self.coach.on_error(ARNOLD_PRESS_SETUP, urgent=True)
            return frame

        self.coach.reset_error(ARNOLD_PRESS_SETUP)
        ...
        # ── TRACKING EXTREMES ──
        # stage "down" means weight is low -> Angle INCREASING as pushed -> track max
        # stage "up" means weight is high -> Angle DECREASING as lowered -> track min
        if self.stage == "down":
        ...
        if current_angle > 160:
            if self.stage == "down":
                ...
                self.coach.reset_error(ARNOLD_PRESS_LOCK)
                self.coach.reset_error(ARNOLD_PRESS_LOWER)
                self.coach.reset_error(ARNOLD_PRESS_DROP)
        ...
        # ── 3) Hysteresis: Ascending (stage=="down", angle increasing) ──
            ...
            if self.max_angle_this_rep - current_angle > 15:
                ...
                voice_message = ARNOLD_PRESS_LOCK
            else:
        ...
        # ── 4) Hysteresis: Descending (stage=="up", angle decreasing) ──
            ...
            if current_angle - self.min_angle_this_rep > 15:
                ...
                current_color = (0, 165, 255)
                voice_message = ARNOLD_PRESS_LOWER
            else:
