...
class InclineDumbbellPressTrainerAI:
    def __init__(self, language="ar"):
        self.counter        = 0
        self.stage          = "up"  # "up" = مفرود لفوق, "down" = الدمبل تحت
        self.feedback       = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            self.coach.on_error(INCLINE_DB_SETUP, urgent=True)
            return frame
        
        self.coach.reset_error(INCLINE_DB_SETUP)
        ...
        if angle > 155:
            if self.stage != "up":
                ...
                self.coach.reset_error(INCLINE_DB_LOCK)
                self.coach.reset_error(INCLINE_DB_LOWER)
                self.coach.reset_error(INCLINE_DB_DROP)
                
                self._reset_rep_tracking()

        # 2) Bottom (Deep stretch)
        elif angle < 85:
            # 🟢 تحذير النزول الزيادة عن اللزوم (DROP) لحماية الكتف
            if angle < 65:
                self.feedback = "DON'T DROP ELBOWS TOO LOW!"
                current_color = (0, 165, 255)
                voice_message = INCLINE_DB_DROP
            else:
                self.coach.reset_error(INCLINE_DB_DROP)
        ...
        # 3) Warning: Not lowering enough (Short rep while descending)
            ...
            # لو عكس اتجاهه لفوث بـ 15 درجة قبل ما ينزل للمستوى الصح
            if angle - self.min_angle_this_rep > 15:
                self.feedback = "LOWER MORE!"
                current_color = (0, 165, 255)
                voice_message = INCLINE_DB_LOWER
            else:
        ...
        # 4) Warning: Partial lock-out while ascending
            ...
            # لو عكس اتجاهه لتحت بـ 15 درجة قبل ما يفرد دراعه
            if self.max_angle_this_rep - angle > 15:
                self.feedback = "LOCK IT OUT!"
                current_color = (0, 165, 255)
                voice_message = INCLINE_DB_LOCK
            else:
