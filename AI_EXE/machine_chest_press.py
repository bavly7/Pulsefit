...
from AI_EXE.messages import (
    ...
    MACHINE_PRESS_LOCK,
    MACHINE_PRESS_DROP,
    GOOD_REP # 🟢 ضفتها عشان التشجيع
)

class MachineChestPressTrainerAI:
    def __init__(self, language="ar"):
        self.counter        = 0
        self.stage          = "up"  # "up" = مفرود, "down" = البار لامس الصدر
        self.feedback       = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            self.coach.on_error(MACHINE_PRESS_SETUP, urgent=True)
            return frame

        self.coach.reset_error(MACHINE_PRESS_SETUP)
        ...
        if angle > 155:
            if self.stage != "up":
                ...
                self.coach.reset_error(MACHINE_PRESS_LOCK)
                self.coach.reset_error(MACHINE_PRESS_LOWER)
                self.coach.reset_error(MACHINE_PRESS_DROP)
        ...
        # 2) Bottom
            ...
            if angle < 65:
                ...
                voice_message = MACHINE_PRESS_DROP
            else:
                self.coach.reset_error(MACHINE_PRESS_DROP)

            if self.stage != "down":
                self.stage = "down"
                self.feedback = "PRESS FORWARD!"
        ...
        # 3) Warning: Not lowering enough (Short rep while descending)
            ...
            # لو هو نازل وقرر يعكس اتجاهه ويطلع بـ 15 درجة قبل ما يوصل للمستوى الصح
            if angle - self.min_angle_this_rep > 15:
                self.feedback = "LET IT BACK MORE!"
                current_color = (0, 165, 255)
                voice_message = MACHINE_PRESS_LOWER
            else:
        ...
        # 4) Warning: Partial lock-out while ascending
            ...
            if self.max_angle_this_rep - angle > 15:
                ...
                current_color = (0, 165, 255)
                voice_message = MACHINE_PRESS_LOCK
            else:
