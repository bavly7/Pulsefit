...
from AI_EXE.messages import (
    SINGLE_LEG_RDL_SETUP,
    SINGLE_LEG_RDL_HINGE,
    SINGLE_LEG_RDL_UP,
...
class SingleLegRdlTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[5] > 0.4 and confs[11] > 0.4 and confs[13] > 0.4 and confs[15] > 0.4:
            ...
            self.coach.on_error(SINGLE_LEG_RDL_SETUP, urgent=True)
            return frame

        self.coach.reset_error(SINGLE_LEG_RDL_SETUP)
        ...
        # ── Form check: standing leg shouldn't bend too much ──────────────
        if knee_angle < 120:
            self.feedback = "DON'T SQUAT IT! HINGE!"
            current_color = (0, 0, 255)
            self.coach.on_error(SINGLE_LEG_RDL_BALANCE)
        else:
            self.coach.reset_error(SINGLE_LEG_RDL_BALANCE)
        ...
        if hip_angle > 165:
            if self.stage == "down":
                ...
                self.coach.reset_error(SINGLE_LEG_RDL_UP)
        ...
        # ── 2) Bottom (Full Hinge / Stretch) ──────────────────────────────
            ...
            if self.stage != "down":
                ...
                self.coach.reset_error(SINGLE_LEG_RDL_HINGE)
                self._reset_rep_tracking()
        ...
        # ── 4) Hysteresis: Ascending (stage=="down", angle still mid-range) ─
            ...
            # Reversal: angle was increasing but DROPPED > 15°
            if self.max_angle_this_rep - hip_angle > 15:
                self.feedback = "LOCK HIPS OUT!"
                current_color = (0, 165, 255)
                self.coach.on_error(SINGLE_LEG_RDL_UP)
            else:
                if knee_angle >= 120:
