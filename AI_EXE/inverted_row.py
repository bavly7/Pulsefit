...
from AI_EXE.messages import (
    INVERTED_ROW_SETUP,
    INVERTED_ROW_PULL,
    INVERTED_ROW_STRETCH,
    INVERTED_ROW_SQUEEZE,
    GOOD_REP # 🟢 عشان التشجيع
)

class InvertedRowTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "down"  # "down" = دراع مفرود, "up" = الصدر لامس البار
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            s_idx, e_idx, w_idx, h_idx, k_idx = 5, 7, 9, 11
        else:
            self.feedback = "SHOW SIDE VIEW"
            self._draw_feedback(frame, (0, 0, 255))
            self.coach.on_error(INVERTED_ROW_SETUP, urgent=True)
            return frame

        self.coach.reset_error(INVERTED_ROW_SETUP)
        ...
        if self.stage == "down":
            # بيشد جسمه لفوق (الزاوية بتقل) -> نسجل أقل زاوية
            self.min_angle_this_rep = min(self.min_angle_this_rep, angle)
        elif self.stage == "up":
            # بينزل جسمه لتحت (الزاوية بتزيد) -> نسجل أكبر زاوية
        ...
        # ── BODY ANGLE CHECK ─────────────────────────────────────────
        # في الانفرتيد رو، لازم الجسم يكون مفرود زي المسطرة (زاوية قريبة من 180)
        if knee is not None:
            body_angle = calculate_angle(shoulder, hip, knee)
            if body_angle < 150: # لو الوسط سقط لتحت
                self.feedback = "KEEP BODY STRAIGHT!"
                current_color = (0, 0, 255)
                self.coach.on_error(INVERTED_ROW_SETUP)
            else:
                self.coach.reset_error(INVERTED_ROW_SETUP)
        else:
            self.coach.reset_error(INVERTED_ROW_SETUP)
        ...
        if angle < 70:
            if self.stage != "up":
                ...
                self.coach.reset_error(INVERTED_ROW_PULL)
                self.coach.reset_error(INVERTED_ROW_STRETCH)
                self.coach.reset_error(INVERTED_ROW_SQUEEZE)
                
                self._reset_rep_tracking()

        # 2) Full extension (start position)
        elif angle > 150:
            if self.stage != "down":
                self.stage = "down"
        ...
        # 3) Warning: Not pulling high enough (short rep while ascending)
        elif self.stage == "down" and 70 < angle < 140:
            if angle - self.min_angle_this_rep > 15:
                self.feedback = "PULL CHEST TO BAR!"
                current_color = (0, 165, 255)
                voice_message = INVERTED_ROW_PULL
            else:
                self.feedback = "PULLING..."
                current_color = (255, 255, 255)

        # 4) Warning: Partial extension while descending
        elif self.stage == "up" and 80 < angle < 150:
            if self.max_angle_this_rep - angle > 15:
                self.feedback = "LOWER FULLY!"
                current_color = (0, 165, 255)
                voice_message = INVERTED_ROW_STRETCH
            else:
