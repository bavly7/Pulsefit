...
from AI_EXE.messages import (
    CABLE_PULL_THROUGH_SETUP,
    CABLE_PULL_THROUGH_PULL,
    CABLE_PULL_THROUGH_SQUEEZE,
    CABLE_PULL_THROUGH_HIP,
    GOOD_REP,
)


class CablePullThroughTrainerAI:
    def __init__(self, language="ar"):
        self.counter  = 0
        self.stage    = "down"        # "down" = hinged/stretched, "up" = lockout
        self.feedback = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        if confs[5] > 0.4 and confs[11] > 0.4 and confs[13] > 0.4 and confs[15] > 0.4:
            ...
            self.coach.on_error(CABLE_PULL_THROUGH_SETUP, urgent=True)
            return frame

        self.coach.reset_error(CABLE_PULL_THROUGH_SETUP)
        ...
        knee_angle = calculate_angle(hip, knee, ankle)   # form check

        current_color = (0, 255, 0)

        # Track extremes
        if self.stage == "up":
            # Descending: hip angle DECREASES → track min
            self.min_angle_this_rep = min(self.min_angle_this_rep, hip_angle)
        else:
            # Ascending: hip angle INCREASES → track max
            self.max_angle_this_rep = max(self.max_angle_this_rep, hip_angle)

        # Form: excessive knee bend (squatting instead of hinging)
        if knee_angle < 120:
            self.feedback = "HINGE, DON'T SQUAT!"
            current_color = (0, 0, 255)
            self.coach.on_error(CABLE_PULL_THROUGH_HIP)
        else:
            self.coach.reset_error(CABLE_PULL_THROUGH_HIP)

        # 1) Lockout (hip fully extended)
        if hip_angle > 165:
            if self.stage == "down":
                self.stage    = "up"
                self.counter += 1
                ...
                self.coach.reset_error(CABLE_PULL_THROUGH_SQUEEZE)
                self._reset_rep_tracking()
                self.max_angle_this_rep = hip_angle
            else:
                if knee_angle >= 120:
                    self.feedback = "SQUEEZE GLUTES!"
                    current_color = (0, 255, 0)

        # 2) Bottom (full hinge / cable stretched back)
        elif hip_angle < 100:
            if self.stage != "down":
                self.stage    = "down"
                self.feedback = "PULL THROUGH!"
                current_color = (255, 255, 255)
                self.coach.reset_error(CABLE_PULL_THROUGH_PULL)
                self._reset_rep_tracking()
        ...
        # 3) Hysteresis: descending (stage=="up")
            ...
            if hip_angle - self.min_angle_this_rep > 15:
                ...
                self.coach.on_error(CABLE_PULL_THROUGH_PULL)
            else:
                if knee_angle >= 120:
        ...
        # 4) Hysteresis: ascending (stage=="down")
            ...
            if self.max_angle_this_rep - hip_angle > 15:
                ...
                current_color = (0, 165, 255)
                self.coach.on_error(CABLE_PULL_THROUGH_SQUEEZE)
            else:
                if knee_angle >= 120:
                    self.feedback = "PULLING THROUGH..."
                    current_color = (255, 255, 255)
                self.coach.reset_error(CABLE_PULL_THROUGH_SQUEEZE)
