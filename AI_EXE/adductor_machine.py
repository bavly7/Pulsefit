...
from AI_EXE.messages import (
    ADDUCTOR_MACHINE_SETUP,
    ADDUCTOR_MACHINE_SQUEEZE,
    ADDUCTOR_MACHINE_OPEN,
    ADDUCTOR_MACHINE_FULL,
    GOOD_REP,
)

class AdductorMachineTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[23] > 0.4 and confs[24] > 0.4 and confs[25] > 0.4 and confs[26] > 0.4:
            ...
            self.coach.on_error(ADDUCTOR_MACHINE_SETUP, urgent=True)
            return frame

        self.coach.reset_error(ADDUCTOR_MACHINE_SETUP)
        ...
        # ── TRACKING EXTREMES ──
        if self.stage == "up":  # closing, angle decreasing
            self.min_angle_this_rep = min(self.min_angle_this_rep, current_angle)
        else:  # opening, angle increasing
        ...
        if current_angle < 45:
            if self.stage == "down":
                ...
                self.coach.reset_error(ADDUCTOR_MACHINE_SQUEEZE)
                self.coach.reset_error(ADDUCTOR_MACHINE_OPEN)
                
                self._reset_rep_tracking()
                self.min_angle_this_rep = current_angle # Seed for closing

        # ── 2) Bottom (Open) ──
        elif current_angle > 120:
            if self.stage != "down":
                self.stage = "down"
                self.feedback = "OPEN LEGS!"
                ...
                self._reset_rep_tracking()
                self.max_angle_this_rep = current_angle # Seed for opening

        # ── 3) Hysteresis: Ascending (Closing, stage=="down") ──
        elif self.stage == "down" and 45 <= current_angle <= 120:
            if current_angle - self.min_angle_this_rep > 15:
                self.feedback = "SQUEEZE TOGETHER!"
                current_color = (0, 165, 255)
                voice_message = ADDUCTOR_MACHINE_SQUEEZE
            else:
                self.feedback = "CLOSING..."
                current_color = (255, 255, 255)

        # ── 4) Hysteresis: Descending (Opening, stage=="up") ──
        elif self.stage == "up" and 45 <= current_angle <= 120:
            if self.max_angle_this_rep - current_angle > 15:
                self.feedback = "OPEN FULLY!"
                current_color = (0, 165, 255)
                voice_message = ADDUCTOR_MACHINE_OPEN
            else:
