...
from AI_EXE.messages import (
    OVERHEAD_TRICEP_DB_SETUP,
    OVERHEAD_TRICEP_DB_PUSH,
    OVERHEAD_TRICEP_DB_STRETCH,
    OVERHEAD_TRICEP_DB_DROP,
    GOOD_REP # 🟢 عشان التشجيع
)

class OverheadTricepDbTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "up"  # "up" = الكوع متني ورا الراس، "down" = الدراع مفرود للسقف
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(OVERHEAD_TRICEP_DB_SETUP, urgent=True)
            return frame

        self.coach.reset_error(OVERHEAD_TRICEP_DB_SETUP)
        ...
        if self.stage == "up":
            # بيزق الدمبل للسقف (الزاوية بتزيد) -> نسجل أكبر زاوية
            self.max_angle_this_rep = max(self.max_angle_this_rep, angle)
        elif self.stage == "down":
            # بينزل الدمبل ورا راسه (الزاوية بتقل) -> نسجل أقل زاوية
            self.min_angle_this_rep = min(self.min_angle_this_rep, angle)

        current_color = (0, 255, 0)
        voice_message = None

        # ── ELBOW POSITION CHECK (منع سقوط الكوع) ────────────────────
        upper_arm_angle = calculate_angle(hip, shoulder, elbow)
        # الكوع لازم يكون مرفوع للسقف (زاوية قريبة من 180 أو على الأقل فوق 140)
        if upper_arm_angle < 140:
            self.feedback = "KEEP ELBOWS POINTING UP!"
            current_color = (0, 0, 255)
            self.coach.on_error(OVERHEAD_TRICEP_DB_DROP)
        else:
            self.coach.reset_error(OVERHEAD_TRICEP_DB_DROP)

            # 1) Lock-out / top (arms extended overhead)
            if angle > 155:
                if self.stage != "down":
                    self.stage    = "down"
                    self.counter += 1
                    ...
                    self.coach.reset_error(OVERHEAD_TRICEP_DB_PUSH)
                    self.coach.reset_error(OVERHEAD_TRICEP_DB_STRETCH)
                    self._reset_rep_tracking()

            # 2) Bottom (deep stretch behind head)
            elif angle < 70:
                if self.stage != "up":
                    self.stage = "up"
