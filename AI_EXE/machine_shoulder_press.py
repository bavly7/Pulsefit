...
from AI_EXE.messages import (
    MACHINE_PRESS_SHOULDER_SETUP,
    MACHINE_PRESS_SHOULDER_LOWER,
    MACHINE_PRESS_SHOULDER_LOCK,
    MACHINE_PRESS_SHOULDER_DROP,
    GOOD_REP,
)

class MachineShoulderPressTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            self.coach.on_error(MACHINE_PRESS_SHOULDER_SETUP, urgent=True)
            return frame

        self.coach.reset_error(MACHINE_PRESS_SHOULDER_SETUP)
        ...
        if current_angle > 160:
            if self.stage == "down":
                ...
                self.coach.reset_error(MACHINE_PRESS_SHOULDER_LOCK)
                self.coach.reset_error(MACHINE_PRESS_SHOULDER_LOWER)
                self.coach.reset_error(MACHINE_PRESS_SHOULDER_DROP)
        ...
        # ── 3) Hysteresis: Ascending (stage=="down", angle increasing) ──
