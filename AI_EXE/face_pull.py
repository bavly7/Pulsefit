...
class FacePullTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(FACE_PULL_SETUP, urgent=True)
            return frame

        self.coach.reset_error(FACE_PULL_SETUP)
        ...
        if self.stage == "down":  # pulling, angle decreasing
            self.min_angle_this_rep = min(self.min_angle_this_rep, current_angle)
        else:  # returning, angle increasing
        ...
        if current_angle < 70:
            if self.stage == "down":
                ...
                self.coach.reset_error(FACE_PULL_PULL)
                self.coach.reset_error(FACE_PULL_STRETCH)
        ...
        # ── 2) Bottom (Stretch) ──
            ...
            if self.stage != "down":
                self.stage = "down"
                self.feedback = "PULL TOWARDS FACE!"
        ...
        # ── 3) Hysteresis: Ascending (stage=="down", pulling) ──
        elif self.stage == "down" and 70 <= current_angle <= 150:
            if current_angle - self.min_angle_this_rep > 15:
                self.feedback = "PULL ELBOWS BACK!"
                current_color = (0, 165, 255)
                voice_message = FACE_PULL_PULL
            else:
                self.feedback = "PULLING..."
                current_color = (255, 255, 255)

        # ── 4) Hysteresis: Descending (stage=="up", returning) ──
        elif self.stage == "up" and 70 <= current_angle <= 150:
            if self.max_angle_this_rep - current_angle > 15:
                self.feedback = "STRETCH FULLY!"
                current_color = (0, 165, 255)
                voice_message = FACE_PULL_STRETCH
            else:
