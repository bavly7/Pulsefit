...
from AI_EXE.messages import (
    DECLINE_PUSHUP_SETUP,
    DECLINE_PUSHUP_LOWER,
    DECLINE_PUSHUP_LOCK,
    DECLINE_PUSHUP_DROP,
    GOOD_REP # 🟢 ضفتها عشان يشتغل التشجيع والتقفيل
)

class DeclinePushUpTrainerAI:
    def __init__(self, language="ar"):
        self.counter        = 0
        self.stage          = "up"  # "up" = مفرود لفوق, "down" = نازل لتحت
        self.feedback       = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4:
            ...
            self.coach.on_error(DECLINE_PUSHUP_SETUP, urgent=True)
            return frame
        
        self.coach.reset_error(DECLINE_PUSHUP_SETUP)
        ...
        if self.stage == "up":
            # وهو نازل لتحت (الزاوية بتقل) نراقب المينيمم
            self.min_angle_this_rep = min(self.min_angle_this_rep, angle)
        elif self.stage == "down":
            # وهو بيزق لفوق (الزاوية بتزيد) نراقب الماكسيمم
        ...
        if angle > 155:
            if self.stage != "up":
                ...
                self.coach.reset_error(DECLINE_PUSHUP_LOCK)
                self.coach.reset_error(DECLINE_PUSHUP_LOWER)
        ...
        # 3) Warning: Not going deep enough (Short rep while descending)
            ...
            # 🟢 لو هو نازل وقرر يعكس اتجاهه ويطلع بـ 15 درجة قبل ما يكمل النزلة للآخر
            if angle - self.min_angle_this_rep > 15:
                self.feedback = "GO LOWER!"
                current_color = (0, 165, 255)
                voice_message = DECLINE_PUSHUP_LOWER
            else:
        ...
        # 4) Warning: Partial extension (Didn't lock out while ascending)
            ...
            # 🟢 لو هو طالع وقرر يريح وينزل تاني بـ 15 درجة قبل ما يفرد دراعه للآخر
            if self.max_angle_this_rep - angle > 15:
                self.feedback = "LOCK ARMS OUT!"
                current_color = (0, 165, 255)
                voice_message = DECLINE_PUSHUP_LOCK
            else:
