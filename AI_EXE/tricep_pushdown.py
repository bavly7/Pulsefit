...
from AI_EXE.messages import (
    TRICEP_PUSHDOWN_SETUP,
    TRICEP_PUSHDOWN_PUSH,
    TRICEP_PUSHDOWN_STRETCH,
    TRICEP_PUSHDOWN_DROP,
    TUCK_ELBOWS,  # 🟢 استدعينا رسالة تثبيت الكوع من ملفك
    GOOD_REP
)
...
class TricepPushdownTrainerAI:
    def __init__(self, language="ar"):
        ...
        # 🟢 التنعيم 1 لسرعة واستجابة الويب
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(TRICEP_PUSHDOWN_SETUP, urgent=True)
            return frame

        self.coach.reset_error(TRICEP_PUSHDOWN_SETUP)
        ...
        # ── Upper arm form check (التحكم الصارم في الكوع) ──────────────
        # 🟢 خلينا الزاوية 25 عشان لو رفع كوعه لقدام سنة يمسكه
        upper_arm_angle = calculate_angle(hip, shoulder, elbow)
        if upper_arm_angle > 25:
            self.feedback = "TUCK YOUR ELBOWS!"
            current_color = (0, 0, 255)
            self.coach.on_error(TUCK_ELBOWS)  # 👈 هيزعقله ويقوله ضم كوعك
        else:
            self.coach.reset_error(TUCK_ELBOWS)

            # 1) Lock-out / extension complete 
            # 🟢 ظبطناها على 150 بدل 160 عشان السرعة
            if angle > 150:
                if self.stage != "down":
                    self.stage    = "down"
                    self.counter += 1
                    ...
                    self.coach.reset_error(TRICEP_PUSHDOWN_PUSH)
                    self.coach.reset_error(TRICEP_PUSHDOWN_STRETCH)
                    self.coach.reset_error(TRICEP_PUSHDOWN_DROP)
                    self._reset_rep_tracking()
                else:
                    self.feedback = "GOOD SQUEEZE!"

            # 2) Full stretch (start position)
            # 🟢 ظبطناها على 90 للاسترتش الكامل
            elif angle < 90:
                if self.stage != "up":
                    self.stage = "up"
            ...
            # 3) Short pushdown warning
            elif self.stage == "up" and 90 <= angle <= 150:
                if self.max_angle_this_rep - angle > 15:
                    self.feedback = "PUSH DOWN FULLY!"
                    current_color = (0, 165, 255)
                    voice_message = TRICEP_PUSHDOWN_PUSH
                else:
                    self.feedback = "PUSHING..."
                    current_color = (255, 255, 255)

            # 4) Partial stretch warning
            elif self.stage == "down" and 90 <= angle <= 150:
                if angle - self.min_angle_this_rep > 15:
                    self.feedback = "STRETCH UP FULLY!"
                    current_color = (0, 165, 255)
                    voice_message = TRICEP_PUSHDOWN_STRETCH
                else:
