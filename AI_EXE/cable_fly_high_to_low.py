...
class CableFlyHighToLowTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(FLY_H2L_SETUP, urgent=True)
            return frame
        
        self.coach.reset_error(FLY_H2L_SETUP)
        ...
        press_angle = self._get_smoothed_angle(raw_angle) # 🟢 تطبيق التنعيم
        ratio       = self._get_smoothed_ratio(raw_ratio)

        # ── Track extremes for direction detection ────────────────────
        # 🟢 تعديل الاتجاهات: وإنت بتضغط لقدام (back) النسبة بتزيد فنسجل الماكسيمم
        if self.stage == "back":
            self.max_ratio_this_rep = max(self.max_ratio_this_rep, ratio)
        # وإنت بترجع (forward) النسبة بتقل فنسجل المينيمم
...
# ── 1. Ratio / Stage Logic (حساب العدة والاتجاه) ──────────────
        if ratio < 0.35:
            if self.stage != "back":
                self.stage    = "back"
                self.feedback = "NOW SQUEEZE DOWN!"
                current_color = (255, 255, 255)
                self._reset_rep_tracking()
                self.coach.reset_error(FLY_H2L_STRETCH)
            else:
                self.feedback = "NOW SQUEEZE DOWN!"
                current_color = (255, 255, 255)

        elif ratio > 0.70:  # 🟢 قللتها سنة لـ 0.70 عشان متبقاش رخمة لو الكاميرا بعيدة
            if self.stage == "back":
                self.stage    = "forward"
                self.counter += 1
                ...
                self.coach.reset_error(FLY_H2L_SQUEEZE)
                self._reset_rep_tracking()
        ...
        # ── 2. Independent Angle Check (فصلنا الكوع لوحده) ────────────
        if press_angle < 100:
            self.feedback = "DON'T BEND ELBOWS!"
            current_color = (0, 165, 255)
            voice_message = FLY_H2L_BENT
        elif press_angle > 115:
            self.coach.reset_error(FLY_H2L_BENT)
