...
class MeadowsRowTrainerAI:
    ...
    def process(self, frame, keypoints, confs):
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(MEADOWS_ROW_SETUP, urgent=True)
            return frame

        self.coach.reset_error(MEADOWS_ROW_SETUP)
        ...
        if angle < 70:
            if self.stage != "up":
                ...
                self.coach.reset_error(MEADOWS_ROW_PULL)
                self.coach.reset_error(MEADOWS_ROW_STRETCH)
                self.coach.reset_error(MEADOWS_ROW_SQUEEZE)
        ...
        # 3) Warning: Not pulling all the way up
            ...
            if angle - self.min_angle_this_rep > 15:
                self.feedback = "PULL HIGH AND WIDE!"
                current_color = (0, 165, 255)
                voice_message = MEADOWS_ROW_PULL
            else:
        ...
        # 4) Warning: Partial stretch while returning down
            ...
            if self.max_angle_this_rep - angle > 15:
                self.feedback = "STRETCH ALL THE WAY!"
                current_color = (0, 165, 255)
                voice_message = MEADOWS_ROW_STRETCH
            else:
