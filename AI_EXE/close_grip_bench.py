...
from AI_EXE.messages import (
    CLOSE_GRIP_BENCH_SETUP,
    CLOSE_GRIP_BENCH_LOWER,
    CLOSE_GRIP_BENCH_PUSH,
    CLOSE_GRIP_BENCH_FLARE,
    GOOD_REP # 🟢 عشان التشجيع
)

class CloseGripBenchPressTrainerAI:
    def __init__(self, language="ar"):
        self.counter    = 0
        self.stage      = "up"  # "up" = دراع مفرود لفوق, "down" = البار على الصدر
        self.feedback   = "Setup"
    ...
    def process(self, frame, keypoints, confs):
        ...
        if confs[6] > 0.4 and confs[8] > 0.4 and confs[10] > 0.4 and confs[12] > 0.4:
            ...
            self.coach.on_error(CLOSE_GRIP_BENCH_SETUP, urgent=True)
            return frame

        self.coach.reset_error(CLOSE_GRIP_BENCH_SETUP)
        ...
        if self.stage == "up":
            # وهو بينزل البار (الزاوية بتقل)
            self.min_angle_this_rep = min(self.min_angle_this_rep, angle)
        elif self.stage == "down":
            # وهو بيزق البار (الزاوية بتزيد)
            self.max_angle_this_rep = max(self.max_angle_this_rep, angle)

        current_color = (0, 255, 0)
        voice_message = None

        # ── ELBOW FLARE CHECK (Tuck your elbows!) ────────────────────
        upper_arm_angle = calculate_angle(hip, shoulder, elbow)
        # لو الكوع فتح أكتر من 45 درجة يبقى بيلعب بنش عادي مش كلوز جريب
        if upper_arm_angle > 45:
            self.feedback = "TUCK YOUR ELBOWS IN!"
            current_color = (0, 0, 255)
            self.coach.on_error(CLOSE_GRIP_BENCH_FLARE)
        else:
            self.coach.reset_error(CLOSE_GRIP_BENCH_FLARE)
            ...
            if angle > 155:
                if self.stage != "up":
                    ...
                    self.coach.reset_error(CLOSE_GRIP_BENCH_LOWER)
                    self.coach.reset_error(CLOSE_GRIP_BENCH_PUSH)
                    self.coach.reset_error(CLOSE_GRIP_BENCH_FLARE)
                    
                    self._reset_rep_tracking()

            # 2) Bottom (chest touch)
            elif angle < 75:
                if self.stage != "down":
                    self.stage = "down"
            ...
            elif self.stage == "up" and 75 <= angle <= 155:
                # لو عكس اتجاهه لفوق 15 درجة قبل ما يوصل للصدر
                if angle - self.min_angle_this_rep > 15:
                    self.feedback = "LOWER TO CHEST!"
                    current_color = (0, 165, 255)
                    voice_message = CLOSE_GRIP_BENCH_LOWER
                else:
                    self.feedback = "LOWERING..."
                    current_color = (255, 255, 255)

            # 4) Warning: Partial lock-out while ascending
            elif self.stage == "down" and 75 <= angle <= 155:
                # لو عكس اتجاهه لتحت 15 درجة قبل ما يفرد دراعه للاخر
                if self.max_angle_this_rep - angle > 15:
                    self.feedback = "PUSH UP AND LOCK!"
                    current_color = (0, 165, 255)
                    voice_message = CLOSE_GRIP_BENCH_PUSH
                else:
